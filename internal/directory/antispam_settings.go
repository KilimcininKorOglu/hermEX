...
func (d *SQLDirectory) GetAntispamSettings() (AntispamSettings, bool, error) {
	...
	err := d.db.QueryRow(
		`SELECT spf_fail, spf_softfail, dkim_fail, dmarc_fail, dnsbl_hit, bayes_spam, sa_rules_hit, threshold, zones, updated_at
		   FROM antispam_settings WHERE id = 1`).Scan(
		&s.SPFFail, &s.SPFSoftFail, &s.DKIMFail, &s.DMARCFail, &s.DNSBLHit, &s.BayesSpam, &s.SARulesHit, &s.Threshold, &s.Zones, &s.UpdatedAt)
...
func (d *SQLDirectory) SetAntispamSettings(s AntispamSettings) error {
	_, err := d.db.Exec(
		`INSERT INTO antispam_settings
		   (id, spf_fail, spf_softfail, dkim_fail, dmarc_fail, dnsbl_hit, bayes_spam, sa_rules_hit, threshold, zones, updated_at)
		 VALUES (1, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
		 ON DUPLICATE KEY UPDATE
		   ...
		   sa_rules_hit=VALUES(sa_rules_hit), threshold=VALUES(threshold), zones=VALUES(zones), updated_at=VALUES(updated_at)`,
		s.SPFFail, s.SPFSoftFail, s.DKIMFail, s.DMARCFail, s.DNSBLHit, s.BayesSpam, s.SARulesHit, s.Threshold, s.Zones, time.Now().UnixMilli())
	return err
}
