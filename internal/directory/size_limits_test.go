...
func TestSizeLimitsRoundTrip(t *testing.T) {
	...
	want := SizeLimits{IMAPLiteralBytes: 10485760} // 10 MiB
...
func TestSizeLimitsUpsert(t *testing.T) {
	d := setupSizeLimits(t)
	if err := d.SetSizeLimits(SizeLimits{IMAPLiteralBytes: 52428800}); err != nil {
		t.Fatal(err)
	}
	if err := d.SetSizeLimits(SizeLimits{IMAPLiteralBytes: 1048576}); err != nil {
		t.Fatal(err)
	}
	got, found, err := d.GetSizeLimits()
	if err != nil || !found {
	...
	if got.IMAPLiteralBytes != 1048576 {
		t.Errorf("after upsert = %+v, want 1048576", got)
	}
}
