...
// privilege_bits (the reference auth model): the service privileges a user holds.
const (
	privIMAPPOP3 = 1 << 0
	privSMTP     = 1 << 1
)

// SQLDirectory is a MariaDB/MySQL-backed account directory, faithful to the reference's
// model (the internal spec): it resolves an address over username/altname/alias,
...
// resolve runs the reference's three-key resolution: the input must match
...
// mirrors the reference resolution: resolve to exactly one row, require an active
...
// its domain is active (the reference recv check). Unknown addresses refuse.
...
// (mainname == users.username), matching the reference aliases table.
...
// same scheme the reference's password hash uses by default.
...
// the reference password verify. An empty stored hash never matches.
