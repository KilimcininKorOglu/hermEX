...
func cleanTables(t *testing.T, db *sql.DB) {
	t.Helper()
	for _, tbl := range []string{"altnames", "aliases", "forwards", "fetchmail_seen", "fetchmail", "admin_roles", "associations", "specifieds", "mlists", "users", "domains", "orgs", "ldap_config", "sync_policy"} {
