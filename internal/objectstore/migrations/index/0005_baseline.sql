-- IMAP/POP3 index baseline (imapindex.sqlite3): per-folder UID allocation
-- (folders.uidnext) and a denormalized per-message row carrying the IMAP uid,
-- flags, and envelope projections for fast listing and search. Folders are keyed
-- by folder_id; the name column is a non-unique display-name projection for
-- diagnostics (sibling folders may legitimately share a display name).
