...
// TestPlainQueryDispatch confirms a plain-query command reaches dispatch. Search
// is not implemented in v1, so it returns 501 — proving the transport and parse
// path works end to end (implemented commands are exercised by the command
// tests).
func TestPlainQueryDispatch(t *testing.T) {
	ts := testServer(t)
	resp, _ := do(t, ts, "POST", "/Microsoft-Server-ActiveSync?Cmd=Search&User="+testUser+"&DeviceId=dev1&DeviceType=iPhone", "", true)
...
// the live endpoint to dispatch (command code 16 = Search, unimplemented in v1).
func TestBase64QueryDispatch(t *testing.T) {
	ts := testServer(t)
	packed := []byte{0x0E, 0x10, 0x09, 0x04, 0x04, 'd', 'e', 'v', '1', 0x00, 0x03, 'A', 'n', 'd'}
