...
// TestPlainQueryDispatch confirms a plain-query command reaches dispatch (and,
// since the command is not yet implemented, returns 501 — proving the transport
// and parse path works end to end).
func TestPlainQueryDispatch(t *testing.T) {
	ts := testServer(t)
	resp, _ := do(t, ts, "POST", "/Microsoft-Server-ActiveSync?Cmd=FolderSync&User="+testUser+"&DeviceId=dev1&DeviceType=iPhone", "", true)
...
// the live endpoint to dispatch.
func TestBase64QueryDispatch(t *testing.T) {
	ts := testServer(t)
	packed := []byte{0x0E, 0x09, 0x09, 0x04, 0x04, 'd', 'e', 'v', '1', 0x00, 0x03, 'A', 'n', 'd'}
