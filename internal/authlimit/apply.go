...
// rather than snapping back to them. daemon names the caller in the log line.
func Apply(daemon string, l *Limiter, read Reader) {
	s, found, err := read()
	if err != nil {
		log.Printf("%s: login-lockout settings read failed, leaving the current tuning in place: %v", daemon, err)
		return
	}
	if !found {
		return
