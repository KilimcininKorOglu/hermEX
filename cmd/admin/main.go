...
var commands = []command{
	...
	{name: "set-password", args: "<email> <password> [--no-force-change] [--keep-sessions]",
		note:    "reset a password; by default the account must change it at the next login and every live session ends",
		minArgs: 3, maxArgs: 5, run: runSetPassword},
	...
	{name: "check-mailbox", args: "<email|all> [--repair] [--recover]",
		note:    "report store damage; --repair rebuilds the IMAP index, --recover rebuilds a damaged database and LOSES the rows on its damaged pages",
		minArgs: 2, maxArgs: 4, run: runCheckMailbox},
	...
	{name: "move-mail", args: "<email> <src-folder> <dst-folder> <uid>... [--copy]",
		note:    "relocate messages between two folders of one mailbox",
		minArgs: 5, run: runMoveMail},
